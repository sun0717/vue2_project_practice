    ...
    <el-card>
      ...
      <div v-show="!isShowTable">
        ...
        >
          ...
          <el-table-column width="200" prop="prop" label="操作">
            <template slot-scope="{ row, $index }">
              <!-- 气泡确认框 -->
              <el-popconfirm
...
export default {
  ...
  methods: {
    ...
    updateAttr(row) {
      //isShowTable变为false
      this.isShowTable = false;
      // 由于数据结构中存在对象里面套数组，数组里套对象，因此需要使用深拷贝解决这类问题
      // 将选中的属性赋值给attrInfo
      this.attrInfo = cloneDeep(row);
      // 在修改某一个属性的时候，将相应的属性值元素添加上flag这个标记
      this.attrInfo.attrValueList.forEach((item) => {
        // 这样写也可以给属性值添加flag自动，但是会发现视图不会跟着变化（因为flag不是响应式数据）
        // 因为Vue无法探测普通的新增property,这样书写的属性并非响应式属性（数据变化视图跟这边）
        // 第一个参数对象：对象 第二个参数：添加新的响应式 第三个参数：新的属性的属性值
    ...
    toEdit(row, index) {
      ...
      // 点击span的时候，重绘重排一个input是需要耗费时间，因此我们不可能一点击span立马获取到input
